...
char m[100]; // Array to store received characters
int i = 0;   // Index to track the position in the received_chars array
...
void on_uart_rx()
{
  while (uart_is_readable(UART_ID)) // which uart generated
  {
    ...
    {
      m[i] = ch; // Null-terminate the string
      i++;
    }
...
int main()
{
  ...
  irq_set_exclusive_handler(UART_IRQ, on_uart_rx); // Set up a RX interrupt
