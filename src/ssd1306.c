/*
To Do List:
- blink the LED on the board and a pixel at 1kHz (keep LED blinking as a heartbeat)
*/
...
// Function to draw a character at a position
void drawChar(int x, int y, char character) {
    ...
    for (int i = 0; i < CHAR_HEIGHT; ++i) {
        for (int j = 0; j < CHAR_WIDTH; ++j) {
            if (charData[i] & (1 << (CHAR_WIDTH - j - 1))) {
...
int main() {
    ...
    int spacing = 2 // CHAR_WIDTH + 1; // Spacing between characters
